...
account_limiter \
    "sevpn" \
    "Softether VPN" \
    "$(vpncmd /server localhost:5555 /password:${MYPASS} /adminhub:${HUB_NAME} /cmd UserList | grep -w 'User Name\|Group Name\|Expiration Date' | sed -z 's/\nExpiration Date/ | Expiration Date/g' | sed -z 's/\nGroup Name/ | Group Name/g' | cut -d '|' -f 2,4,6 | sed '/private/d' | sed '/trial_/d' | wc -l)"

# DATE EXPIRED
DATEEXP=`date -d "${days} days" +"%Y/%m/%d %H:%M:%S"`
SDATEEXP=`date -d "${days} days" +"%d %B %Y"`
...
function sevpn_creator(){
    # Execution
    vpncmd /server localhost:5555 /password:${MYPASS} /adminhub:${HUB_NAME} /cmd UserCreate ${username} /GROUP:${segroup} /REALNAME:${username} /NOTE:${username} &> /dev/null
    ...
    unzip -o -C /etc/gegevps/softether/se-ovpn.zip -d /etc/gegevps/softether &>/dev/null
