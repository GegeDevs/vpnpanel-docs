...
account_limiter \
    "trojango" \
    "Trojan-Go" \
    "$(expr $(cat "${trojango_workdir}/userpass.txt" | sed '/^$/d' | wc -l) + 1)"
...
# Check Expired entry
if [[ ! "$(num_check ${expires})" == 'ok' ]]; then
    echo -e " Invalid Expire Days"
    echo -e " ${expires} not a Number"
    exit 1
fi

# User Expired (Timestamp)
user_timestamp="$(date -d "${expires} hours" +"%s")"
...
echo -e "Expired : $(date -d "@${user_timestamp}" +"%Y %B %d %H:%M:%S %Z")"
echo -e "━━━━━━━━━━━━━━━━"
